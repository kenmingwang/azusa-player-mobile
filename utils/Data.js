...
import VideoInfo from "../object/VideoInfo";
...
// Header GIF base
const URL_HEADER_GIF =
	"https://github.com/kenmingwang/azusa-player-lrcs/blob/main/aziRandomPic/{count}.gif?raw=true";
// HEADER GIFs count: https://github.com/kenmingwang/azusa-player-lrcs/tree/main/aziRandomPic
const COUNT_HEADER_GIFS = 12;
// QQ SongSearch API
const URL_QQ_SEARCH =
	"https://c.y.qq.com/soso/fcgi-bin/client_search_cp?g_tk=938407465&uin=0&format=json&inCharset=utf-8&outCharset=utf-8&notice=0&platform=h5&needNewCode=1&w={KeyWord}&zhidaqu=1&catZhida=1&t=0&flag=1&ie=utf-8&sem=1&aggr=0&perpage=20&n=20&p=1&remoteplace=txt.mqq.all&_=1459991037831";
// QQ LyricSearchAPI
const URL_QQ_LYRIC =
...
export const fetchVideoInfo = async (bvid) => {
	...
	try {
		...
		const v = new VideoInfo(
			...
			data.owner,
			data.pages.map((s) => {
				return { bvid: bvid, part: s.part, cid: s.cid, duration: s.duration };
			})
		);
		return v;
	} catch (error) {
...
export const fetchFavList = async (mid) => {
	...
	const res = await fetch(
		URL_FAV_LIST.replace("{mid}", mid).replace("{pn}", 1)
	);
	const json = await res.json();
	const data = json.data;
	...
	for (let page = 2; page <= totalPagesRequired; page++) {
		pagesPromises.push(
			fetch(URL_FAV_LIST.replace("{mid}", mid).replace("{pn}", page))
		);
	}
...
export const getRandomHeaderGIF = () => {
	return URL_HEADER_GIF.replace(
		"{count}",
		Math.floor(Math.random() * COUNT_HEADER_GIFS)
	);
};
...
export const searchLyricOptions = async (searchKey, setOptions, setLyric) => {
	...
	const slimData = data.map((s, v) => {
		return {
			key: s.songmid,
			songMid: s.songmid,
			label: v + ". " + s.songname + " / " + s.singer[0].name,
		};
	});
